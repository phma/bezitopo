 ...
 */
{
  ...
  for (i=260;i<300;i++)
    tassert(tally[i]==0);
  tassert(loop.size()==16); // 18 if all pinch points were split, 15 if none
}
...
void testproperty()
{
  ...
  tassert(dprop==5);
  tassert(q.hasProperty(PROP_LENGTH));
  dprop=q.getDoubleProperty(PROP_LENGTH);
  cout<<"q's length is "<<dprop<<endl;
  tassert(fabs(dprop-5.236)<1e-3);
  ...
  tassert(fabs(dprop-5)<1e-6); // roundoff error because 60° isn't exact
  ...
  tassert(dist(xyprop,xy(-1.964,4.598))<1e-3);
  ...
  tassert(lin.hasProperty(PROP_CENTER));
  xyprop=lin.getXyProperty(PROP_CENTER);
  cout<<"lin's center is ("<<xyprop.getx()<<','<<xyprop.gety()<<")\n";
  tassert(!xyprop.isfinite());
...
void testobjlist()
{
  ...
  psegment=new segment(xyz(doc.modelSpace[nwPipe].obj->getXyProperty(PROP_CENTER),270.295),
		       xyz(doc.modelSpace[nePipe].obj->getXyProperty(PROP_CENTER),245.22541));
  nBoundary=doc.modelSpace.insert(psegment);
...
void testcurly()
/* This outputs "bogus spiralarc" as it draws the maximum-length spiralarc.
 * The endpoints are sufficiently inaccurate that approx3d complains.
 */
{
  int i;
  ...
  test1curly(10,55,ps,times,0.24987184178648592,0.40780786562063787, 1.805132852105974,2.0567381143999013,-1);
  test1curly(10,36,ps,times,0.2657568235453367,0.44184285847891247, 2.1378953591017407,2.448113766254228,-1);
  cout<<ldecimal(1e3*times[1]/times[0],0.01)<<" µs per curly test\n";
  cout<<ldecimal(1e3*times[2]/times[0],0.01)<<" µs per too curly test\n";
}

void test1curvefit(vector<xyz> points,Circle startLine,Circle endLine,double toler,double firstlength,PostScript &ps)
{
  vector<xy> points2d;
  deque<Circle> lines;
  int i,j;
  ...
  ps.endpage();
  cout<<"First length "<<ldecimal(apx.getarc(0).length())<<endl;
  tassert(isnan(firstlength) || fabs(apx.getarc(0).length()-firstlength)<1e-6);
  cout<<apx.size()<<" arcs, error "<<curvefitMaxError(apx,points2d)<<endl;
  tassert(curvefitMaxError(apx,points2d)<toler);
}
 ...
 */
{
  ...
  test1curvefit(points,startLine,endLine,0.3,NAN,ps);
  points.clear();
  ...
  test1curvefit(points,startLine,endLine,0.0001,straightLength,ps);
}
